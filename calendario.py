...
def calendario_eventos():
    ...
    for e in eventos_mostrados:
        ...
        tipo_info = e["asociado"]
        color = "#1f77b4" if e["tipo"] == "Chofer" else "#ff7f0e"
        eventos_formateados.append({
            "id": e["id"],
            "title": f'{e["asunto"]} ({tipo_info})',
    ...
    if st.session_state.editando_evento is None:
        ...
        with st.form("form_crear"):
            ...
            if tipo == "Chofer":
                asociado = st.text_input("Nombre del chófer")
            else:
                asociado = st.text_input("Matrícula del camión o remolque")
    ...
    if not eventos_mostrados:
        ...
        for i, e in enumerate(eventos_mostrados):
